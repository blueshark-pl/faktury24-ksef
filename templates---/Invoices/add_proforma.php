 ...
 */
$this->assign('title', 'Wystaw fakturę proforma');
...
<div class="my-4 page-header-breadcrumb d-flex align-items-center justify-content-between flex-wrap gap-2">
  <div>
    <h1 class="page-title fw-medium fs-18 mb-2">Wystaw fakturę proforma</h1>
    <ol class="breadcrumb mb-0">
      ...
      <li class="breadcrumb-item active" aria-current="page">Wystaw fakturę proforma</li>
    </ol>
  </div>
  <div class="btn-list">
  ...
  <div class="col-xxl-12">
    <div class="card custom-card">
      <div class="card-header d-md-flex d-block">
        <div class="card-title">Wystaw fakturę proforma</div>
...
if ($.fn && $.fn.select2) {
  var $series = $('#series-select').select2({
    placeholder: 'Wybierz schemat numeracji (proforma)',
    allowClear: true,
    width: '100%',
    ajax: {
      url: seriesSearchUrl,
      dataType: 'json',
      delay: 200,
      data: function (params) { return { q: (params.term || ''), limit: 20, type: 'proforma' }; },
      processResults: function (data) {
  ...
  (function preloadSeries(){
    // Na starcie nie przyjmujemy wartości z kontrolera (może być nie-proforma),
    // tylko dociągamy listę dla type=proforma i wybieramy odpowiednią.
...
// Po otwarciu modala „Dodaj schemat numeracji” – automatycznie wybierz typ numeracji = proforma (jeśli istnieje)
$('#series-create-modal').on('shown.bs.modal', function(){
  var $typeSel = $('#series-type-select');
  if (!($.fn && $.fn.select2)) return;
  // Spróbuj odszukać typ po frazie „proforma” i wybrać pierwszy wynik
  $.ajax({ url: seriesTypesUrl, dataType: 'json', data: { q: 'proforma', limit: 10 }})
    .done(function(res){
      var list = (res && res.results) || [];
      if (list.length){
        var it = list[0];
        var opt = new Option(it.text || it.name || 'proforma', it.id || it.value || it.text, true, true);
