  ...
  function fillContractorSnapshot(c){
    ...
    setTimeout(function(){
      if (window.jQuery && jQuery.fn.countrySelect) {
        ...
        if ($vpUI.length) try { $vpUI.countrySelect('selectCountry', (data.vat_prefix || '').toLowerCase()); } catch(e) {}
  ...
  (function initInvCountryPickers() {
    if (!window.jQuery || !jQuery.fn.countrySelect) {
      ...
      var t = setInterval(function() {
        ...
        else if (waited > 60) clearInterval(t);
      }, 50);
      return;
    }
    ...
    if ($vpUI.length && !$vpUI.data('cs-inited')) {
      ...
      $vpUI.on('change', function() { try { var d = $vpUI.countrySelect('getSelectedCountryData'); if (d && d.iso2) $vpH.val(d.iso2.toUpperCase()); } catch(e) {} });
