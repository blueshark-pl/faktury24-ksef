 ...
 */
$this->assign('title', __('Integracja KSeF – szczegóły'));
?>

<div class="my-4 page-header-breadcrumb d-flex align-items-center justify-content-between flex-wrap gap-2">
  <div>
    <h1 class="page-title fw-medium fs-18 mb-2">
      <?= __('Integracja z Krajowym Systemem e-Faktur (KSeF)') ?>
    </h1>
    <nav>
      <ol class="breadcrumb mb-0">
        <li class="breadcrumb-item"><?= $this->Html->link(__('Integracja KSeF'), ['action'=>'index']) ?></li>
        <li class="breadcrumb-item active"><?= __('Szczegóły tokenu') ?></li>
      </ol>
    </nav>
  </div>
  ...
  <div class="col-12 col-lg-8">
    <div class="card custom-card border-primary shadow-sm">
      <div class="card-header bg-primary text-white d-flex align-items-center">
        ...
        <div class="card-title mb-0"><?= __('Szczegóły tokenu KSeF') ?></div>
      </div>
      ...
      <div class="card-body">
        <dl class="row mb-0 align-items-center">
          ...
          <dd class="col-sm-8">
            ...
            <?php else: ?>
              <span class="badge bg-secondary-subtle text-muted px-3 py-2">
                <i class="ri-more-line me-1"></i><?= h($auth->status) ?>
              </span>
            <?php endif; ?>
          ...
          <?php if (!empty($auth->auth_method)): ?>
            <dt class="col-sm-4"><?= __('Metoda autoryzacji') ?></dt>
            <dd class="col-sm-8">
              <?= match($auth->auth_method) {
                'pz' => 'Podmiot zaufany (PZ)',
                'qualified_sign' => 'Podpis kwalifikowany',
                'qualified_seal' => 'Pieczęć kwalifikowana',
                default => h($auth->auth_method)
              } ?>
            </dd>
          <?php endif; ?>
  ...
  <div class="col-12 col-lg-4">
    <div class="card custom-card border-0 shadow-sm">
      ...
      <div class="card-body small text-muted">
        <p>
          Token autoryzacyjny został zapisany w systemie w formie zaszyfrowanej,
          przy użyciu indywidualnego klucza ochrony aplikacji.
          Widoczny jest wyłącznie jego końcowy fragment (ostatnie 4 znaki),
          co umożliwia identyfikację bez ujawniania pełnej wartości.
        </p>
        <p>
          Dane tokenu są wykorzystywane wyłącznie do komunikacji z
          <strong>Krajowym Systemem e-Faktur</strong> w imieniu Twojej firmy.
        </p>
        <hr>
        <p class="small mb-1"><strong><?= __('Dodatkowe informacje:') ?></strong></p>
        <ul class="small mb-0">
          <li><?= __('Każdy nowy token dezaktywuje poprzedni.') ?></li>
          <li><?= __('Dostęp do tokenów mają tylko użytkownicy o roli administracyjnej.') ?></li>
          <li><?= __('Klucze szyfrujące przechowywane są poza bazą danych.') ?></li>
        </ul>
      </div>
    </div>
  </div>
    ...
    <div class="modal-content">
      <div class="modal-header border-0">
        <h5 class="modal-title" id="deactivateLabel">
          <i class="ri-lock-line me-1 text-danger"></i> <?= __('Dezaktywuj token KSeF') ?>
        </h5>
      ...
      <div class="modal-body">
        ...
        <p class="text-muted small mb-0">
          <?= __('System przestanie używać tego tokenu do połączeń z KSeF. Odcisk:') ?>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  modal.addEventListener('show.bs.modal', function (event) {
    ...
    const token = btn.getAttribute('data-label') || '••••';
