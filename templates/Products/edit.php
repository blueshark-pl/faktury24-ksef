 ...
 */
$this->assign('title', 'Edytuj produkt / usługę');

// Czy sekcja zaawansowana ma wartości? Jeśli tak — rozwiń domyślnie
$hasAdvanced = !empty($product->gtin) || !empty($product->cn_code) || !empty($product->pkob)
  || !empty($product->excise_amount) || !empty($product->procedure_marking)
  || !empty($product->is_attachment15);
?>
...
<div class="container-fluid px-0">
  <div class="d-flex align-items-center gap-2 mb-3">
    ...
    <h4 class="mb-0">Edytuj: <?= h($product->name) ?></h4>
    <?= $this->Form->postLink(
      '<i class="ri-delete-bin-line me-1"></i> Usuń',
      ['action' => 'delete', $product->id],
      [
        'escape' => false,
        'confirm' => 'Na pewno usunąć produkt "' . h($product->name) . '"?',
  ...
  <?= $this->Form->create($product, ['class' => 'row g-3']) ?>
      ...
      <div class="accordion" id="acc-advanced">
        <div class="accordion-item border">
          <h2 class="accordion-header">
            <button class="accordion-button <?= $hasAdvanced ? '' : 'collapsed' ?> fw-semibold" type="button"
                    data-bs-toggle="collapse" data-bs-target="#pane-advanced"
                    aria-expanded="<?= $hasAdvanced ? 'true' : 'false' ?>">
              <i class="ri-settings-3-line me-2"></i> Zaawansowane (JPK / KSeF)
              <?php if ($hasAdvanced): ?>
                <span class="ms-2 badge bg-info text-dark fw-normal" style="font-size:.75rem;">wypełnione</span>
              <?php else: ?>
                <span class="ms-2 badge bg-secondary fw-normal" style="font-size:.75rem;">opcjonalne</span>
              <?php endif ?>
            </button>
          </h2>
          <div id="pane-advanced" class="accordion-collapse collapse <?= $hasAdvanced ? 'show' : '' ?>">
            <div class="accordion-body">
              <div class="row g-3">
                ...
                <div class="col-12 d-flex gap-3 align-items-center mt-2">
                  ...
                  <div class="form-check">
                    <?= $this->Form->control('deleted', ['type' => 'checkbox', 'label' => 'Usunięty (archiwum)', 'class' => 'form-check-input']) ?>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div class="col-12 d-flex gap-2">
      <?= $this->Form->button('<i class="ri-save-line me-1"></i> Zapisz zmiany', ['type' => 'submit', 'escape' => false, 'class' => 'btn btn-primary']) ?>
