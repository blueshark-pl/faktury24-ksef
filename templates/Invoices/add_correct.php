...
$this->assign('title', $__isEdit ? 'Edytuj korektę' : 'Wystaw fakturę');
...
// Prefill danych w trybie edycji — z zapisanej korekty
...
<script>
  window._originalPrefill = {
    ...
    currency: <?= json_encode((string)($original->currency ?? 'PLN')) ?>
  };

  $(function(){
    // Defer prefill — addItemRow() jest rejestrowane w innym $(function(){})
    setTimeout(function(){
    try {
      ...
      (function(){
        ...
        function applyCountry(){
          ...
          if (!country || !$c.length) return;
          $c.val(country);
          try { $c.trigger('change'); } catch(e){}
          try { $c.trigger('change.select2'); } catch(e){} // wymuś re-render tekstu Select2 (był stary „PL — Polska")
        }
        applyCountry();
      ...
      if (Array.isArray(d.items) && d.items.length) {
        ...
        var $firstRow = $tbody.find('tr.item-row').first();
        fillRow($firstRow, items[0]);
  ...
  async function fetchNbpRate(){
    ...
    try {
      ...
      if (json && json.success && json.rate){
        ...
        // Debug + diff notice vs original invoice rate
...
if ($.fn && $.fn.select2) {
  ...
  // Ładowanie domyślnej serii — typ zależny od rodzaju korekty, fallback do wszystkich
  function loadDefaultSeries() {
    var d = { q: '', limit: 50 };
    if (kind === 'currency') d.type = 'currency';
    else if (kind === 'vat') d.type = 'vat';
    $.ajax({ url: seriesSearchUrl, dataType: 'json', data: d })
      .done(function(data) {
