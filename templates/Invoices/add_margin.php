        ...
        <div class="p-3 bg-light border rounded mb-3">
          <div class="row g-3 align-items-start">
            <div class="col-xl-8">
              ...
              <!-- Snapshot kontrahenta (invoice_contractors) — UKRYTY NA START, rozwinięty w trybie edycji -->
...
<?php foreach ($__prefillItems as $__i => $__it):
    ...
    foreach (($gtuOptions ?? []) as $__gval => $__glabel) {
        ...
        $__gtuOptionsHtml .= '<option value="'.h($__gval).'"'.$__sel.'>'.h($__glabel).'</option>';
    }
...
<tr>
  <td>
    ...
    <input type="hidden" name="items[<?= (int)$__i ?>][cn_code]" class="item-cn_code" value="<?= h($__it['cn_code'] ?? '') ?>">
  ...
  <td class="gtu-cell"><select class="form-select item-gtu" name="items[<?= (int)$__i ?>][gtu_code]"><?= $__gtuOptionsHtml ?></select></td>
  <td><input name="items[<?= (int)$__i ?>][price]" type="number" step="0.01" value="<?= h(number_format((float)($__it['price'] ?? 0), 2, '.', '')) ?>" class="form-control text-end item-amount" required></td>
  <td><input name="items[<?= (int)$__i ?>][purchase_price]" type="number" step="0.01" value="<?= h(number_format((float)($__it['purchase_price'] ?? 0), 2, '.', '')) ?>" class="form-control text-end item-purchase" placeholder="wew."></td>
...
<?php endif; ?>
  ...
  <td colspan="2">
    <table class="table table-sm text-nowrap mb-0 table-borderless">
      <tbody>
        <tr>
          <th scope="row"><div class="fw-medium">Zakup (suma kosztów) :</div></th>
        ...
        <tr class="d-none" aria-hidden="true">
          <th scope="row"><div class="fw-medium">VAT od marży :</div></th>
          <td><input type="text" id="sum-tax" class="form-control invoice-amount-input text-end" value="0.00" readonly></td>
        </tr>
        <tr>
          <th scope="row"><div class="fs-14 fw-medium">Sprzedaż brutto :</div></th>
...
$('#gus-fetch-btn').on('click', function(){
    ...
    .on('select2:select', function (e) {
      ...
      if (String(d.id || '').indexOf('NEW:') === 0) {
        ...
        if (typeof d.price !== 'undefined' && d.price !== null && d.price !== '') {
          $tr.find('.item-amount').val(Number(d.price).toFixed(2));
        }
  ...
  // ====== INICJALIZACJA WIERSZY (pierwszy lub wszystkie w edit) ======
  (function initExistingRows(){
    var $rows = $itemsBody.find('tr').has('.item-product-select');
    $rows.each(function(){
      var $tr = $(this);
      ...
      $tr.find('.btn-remove').on('click', function(){
        var rows = $itemsBody.find('tr').has('.item-product-select').length;
        if(rows > 1){ $tr.remove(); allCalc(); guardMinRows(); }
      });
    });
    // idx powinien startować za ostatnim istniejącym wierszem
...
if ($.fn && $.fn.select2) {
  var $series = $('#series-select').select2({
    ...
    ajax: {
      ...
      dataType: 'json',
      delay: 200,
      data: function (params) { return { q: (params.term || ''), limit: 20, type: 'margin' }; },
      processResults: function (data) {
        ...
        // Automatycznie wybierz domyślną serię przy pierwszym załadowaniu (tylko type=margin)
  ...
  // Ładowanie domyślnej serii — typ margin, fallback do wszystkich
  function loadDefaultSeries() {
    $.ajax({ url: seriesSearchUrl, dataType: 'json', data: { q: '', limit: 50, type: 'margin' } })
      .done(function(data) {
