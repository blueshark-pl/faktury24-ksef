...
<?= $this->Form->hidden('kind', ['value' => 'novat']) ?>
  ...
  <div class="col-xxl-12">
    <div class="card custom-card">
      <div class="card-header d-md-flex d-block">
        <div class="card-title">Wystaw rachunek</div>
        <div class="ms-auto mt-md-0 mt-2">
            <?php if ($_isAdmin || $_role !== 'user'): ?>
              <?= $this->Form->button('Zapisz jako roboczą', ['class' => 'btn btn-sm btn-outline-secondary', 'name' => 'save_draft']) ?>
              ...
              <?php else: ?>
                <?= $this->Form->button('Zapisz i wystaw', ['class' => 'btn btn-sm btn-primary ms-1', 'name' => 'save_only']) ?>
              <?php endif; ?>
            <?php else: ?>
              <?= $this->Form->button('Zapisz jako roboczą', ['class' => 'btn btn-sm btn-primary', 'name' => 'save_draft']) ?>
            <?php endif; ?>
        </div>
      </div>
  ...
  <td colspan="4">
    <table class="table table-sm text-nowrap mb-0 table-borderless">
      ...
      <div class="card-footer text-end">
          <?php if ($_isAdmin || $_role !== 'user'): ?>
            <?= $this->Form->button('Zapisz jako roboczą', ['class' => 'btn btn-outline-secondary m-1', 'name' => 'save_draft']) ?>
            <?php if ($__ksefModeEnabled): ?>
              <?= $this->Form->button('Zapisz i wyślij do KSeF <i class="ri-send-plane-line ms-1 align-middle d-inline-block"></i>', [
                'class' => 'btn btn-primary m-1', 'escapeTitle' => false, 'name' => 'save_and_send_ksef'
              ]) ?>
            <?php else: ?>
              <?= $this->Form->button('Zapisz i wystaw', ['class' => 'btn btn-primary m-1', 'name' => 'save_only']) ?>
            <?php endif; ?>
          <?php else: ?>
...
$(function () {
  ...
  // Zamknij popover po kliknięciu poza nim
  document.addEventListener('click', function(e) {
    var el = document.getElementById('vat-help');
    if (!el) return;
...
$('#gus-fetch-btn').on('click', function(){
  ...
  ['catalog-help','fp-help','currency-help','autosend-help'].forEach(function(id){
    ...
    if (el) new bootstrap.Popover(el, {container:'body', sanitize:false, trigger:'click'});
  });
