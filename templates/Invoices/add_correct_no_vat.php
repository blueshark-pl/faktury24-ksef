 ...
 * @var \App\Model\Entity\Invoice|null $original
 ...
 */
$this->assign('title', 'Korekta faktury (bez VAT)');
...
<div class="my-4 page-header-breadcrumb d-flex align-items-center justify-content-between flex-wrap gap-2">
  <div>
    <h1 class="page-title fw-medium fs-18 mb-2">Korekta faktury (bez VAT)</h1>
    <ol class="breadcrumb mb-0">
      ...
      <li class="breadcrumb-item active" aria-current="page">Korekta faktury (bez VAT)</li>
    </ol>
  </div>
  <div class="btn-list">
  ...
  <div class="col-xxl-12">
    <div class="card custom-card">
      <div class="card-header d-md-flex d-block">
        <div class="card-title">Korekta faktury (bez VAT)</div>
...
<?php endif; ?>
  ...
  <td colspan="8" class="border-bottom-0">
    <button type="button" class="btn btn-light" id="btn-add-item"><i class="bi bi-plus-lg"></i> Dodaj pozycję korekty</button>
  </td>
</tr>
  ...
  <td colspan="4">
    <table class="table table-sm text-nowrap mb-0 table-borderless">
      <tbody>
        <tr>
          <th scope="row"><div class="fw-medium">Razem netto korekty:</div></th>
          <td><input type="text" id="sum-net" class="form-control invoice-amount-input text-end" value="0.00" readonly></td>
        </tr>
        <tr>
          <th scope="row"><div class="fs-14 fw-medium">Razem brutto korekty:</div></th>
        ...
        <?php if (!empty($original)): ?>
        <div class="mt-3">
          <?= $this->Form->control('correction_reason', [ 'label' => 'Powód korekty', 'type' => 'textarea', 'rows' => 2, 'class' => 'form-control', 'placeholder' => 'Krótko opisz przyczynę korekty' ]) ?>
        </div>
        <div class="mt-2">
          <?= $this->Form->control('correction_type', [ 'label' => 'Typ korekty (bez VAT)', 'type' => 'select', 'class' => 'form-select', 'options' => [ '' => '— Wybierz typ —', 'net' => 'Zmiana podstawy (netto)', 'quantity' => 'Zmiana ilości', 'return' => 'Zwrot/odebranie towaru/usługi' ], 'empty' => false, 'value' => $invoice->correction_type ?? 'net' ]) ?>
          <small class="text-muted">Dotyczy tylko skutków bez VAT (brutto = netto).</small>
        </div>
        <?php endif; ?>
      </div>
...
$('#gus-fetch-btn').on('click', function(){
  ...
  function allCalc(){
    ...
    // Dodatkowo: jeśli mamy oryginał, pokaż różnicę
    var $diffNet = $('#diff-sum-net');
    var $origNet = $('#orig-sum-net');
    if ($diffNet.length && $origNet.length) {
  ...
  $('#product-create-form').on('submit', function (e) {
    ...
    $.ajax({
      ...
      if (data && data.success && currentProductRow && data.product) {
        ...
        currentProductRow.find('.item-price').val(price.toFixed(2));
  ...
  $form.on('submit', function (e) {
    ...
    // Korekta: upewnij się, że mamy powód i typ jeśli oryginał istnieje
    <?php if (!empty($original)): ?>
    var reason = $('[name="correction_reason"]').val()||'';
    var ctype  = $('[name="correction_type"]').val()||'';
    if (!reason.trim()) { e.preventDefault(); e.stopPropagation(); toast('Podaj powód korekty.'); return false; }
    if (!ctype.trim())  { e.preventDefault(); e.stopPropagation(); toast('Wybierz typ korekty.'); return false; }
    <?php endif; ?>
  ...
  <?php if (!empty($original) && empty($isEdit)): ?>
    try {
      var d = {
        ...
        items: <?= json_encode(array_map(function($it){
          return [
            ...
            'price' => isset($it->price) ? (float)$it->price : (isset($it->net_price) ? (float)$it->net_price : 0),
      ...
      if (Array.isArray(d.items) && d.items.length) {
        ...
        for (var i = 0; i < d.items.length; i++) {
          var it = d.items[i]; var $tr = $(rows[i]);
          $tr.find('.item-name-hidden').val(it.name||'');
          $tr.find('.item-qty').val(it.quantity||1);
          $tr.find('.item-price').val(Number(it.price||0).toFixed(2));
          $tr.find('.item-disc').val(it.discount_percent||0);
          var $psel = $tr.find('.item-product-select');
          if ($psel.length) {
            var pid = 'NEW:' + (it.name||('Pozycja '+(i+1)));
            var opt = new Option(it.name||('Pozycja '+(i+1)), pid, true, true);
    ...
    } catch(e) { console.warn('Prefill correction (noVAT) failed', e); }
  <?php endif; ?>
