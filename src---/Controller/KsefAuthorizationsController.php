            ...
            if ($loadOk) {
                ...
                if ($nodes && $nodes->length) {
                    foreach ($nodes as $n) {
                        ...
                        // Support FA(3) tag names (P_7, P_8A, P_8B, P_9A, P_11, P_12, NrWierszaFa) plus generic names
                        $name = $get(['P_7','NazwaTowaruUslugi','Nazwa','OpisPozycji','Opis','PozycjaNazwa']);
                        $qty  = $get(['P_8A','Ilosc','LiczbaJednostek','Quantity']);
                        $unit = $get(['P_8B','JednostkaMiary','Jednostka','Unit']);
                        $price= $get(['P_9A','CenaJednostkowa','CenaJedn','UnitPrice']);
                        $net  = $get(['P_11','WartoscNetto','Netto','NetAmount']);
                        $vatR = $get(['P_12','StawkaPodatku','StawkaVAT','VatRate']);
                        $vatA = $get(['KwotaVat','KwotaVAT','VatAmount']);
                        $grs  = $get(['WartoscBrutto','Brutto','GrossAmount']);
                        $currency = $get(['Waluta','Currency']);
                        $lineId = $get(['NrWierszaFa','NrWiersza','LineId']);
                        ...
                        $items[] = [
                            ...
                            'net_amount' => $toFloat($net),
                            'vat_rate' => $vatR,
                            'vat_amount' => $toFloat($vatA),
                            'gross_amount' => $toFloat($grs),
                            'currency' => $currency,
                            'saved' => $isSaved,
                            'cost_type' => $savedType,
                            'note' => $savedNote ?? null,
