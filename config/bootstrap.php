...
// ─── Tryb serwisowy KSeF ────────────────────────────────────────────────────
// Dostęp wyłącznie dla administratora. Wszyscy inni widzą komunikat serwisowy.
(function () {
    $allowedIp = '188.147.249.109';

    // Obsługa proxy / load-balancer (X-Forwarded-For)
    ...
    if ($ip === $allowedIp) {
        return; // administrator — normalny dostęp
    }

    // CLI (cron, migrations) – nie blokuj
    if (PHP_SAPI === 'cli') {
        return;
    }
 ...
 * See: https://github.com/FriendsOfCake/CakePdf
 */
Configure::write('CakePdf', [
    // Use DomPdf engine (vendor: dompdf/dompdf)
    ...
    'download' => false, // inline by default; controller can override
    'paper' => 'A4',
]);
