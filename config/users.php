...
return [
    ...
    // ── RBAC Provider: DbProvider (feature flag, DEFAULT OFF) ─────────────
    // Gdy Configure Auth.dbRbacEnabled = true, uprawnienia SA sprawdzane
    // wg tabeli roles_permissions (mapa: config/permissions_map.php).
    // Gdy false (default) - zachowanie identyczne z ConfigProvider (permissions.php).
    //
    // Wlaczenie: dopisz do config/app_local.php:
    //   'Auth' => ['dbRbacEnabled' => true],
    //
    // Provider zawsze dolacza reguly z config/permissions.php (jako fallback +
    // wildcard admin), wiec nie ma ryzyka lock-out po wlaczeniu flag.
    'Auth.RbacPolicy' => [
        'adapter' => [
            'className' => \CakeDC\Auth\Rbac\Rbac::class,
            'permissions_provider_class' => \App\Rbac\Permissions\DbProvider::class,
        ],
    ],
];